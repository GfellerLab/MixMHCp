...
.AA_NAMESPACE = function() c('A', 'R', 'N', 'D', 'C', 'Q', 'E', 'G', 'H', 'I', 'L', 'K', 'M', 'F', 'P', 'S', 'T', 'W', 'Y', 'V',
                             'a', 'r', 'n', 'd', 'c', 'q', 'e', 'g', 'h', 'i', 'l', 'k', 'm', 'f', 'p', 's', 't', 'w', 'y', 'v') #### EDITED by MS
...
computeBits <- function(pwm, N=4, Nseqs=NULL, smallSampleCorr=TRUE){
  Nseqs = attr(pwm, 'nongapped')
  H_i = - apply(pwm, 2, function(col) sum(col * log2(col), na.rm=T))
  e_n = 0

  #### EDITED by MS: decided by the user if small sample correction
  if(smallSampleCorr){
...
makePFM <- function(seqs, seq_type='auto', namespace=NULL, keep_letter_mat=F, smallSampleCorr=TRUE, additionalAA=NULL, ...){
  ...
  #### EDITED by MS: N not namesspaces, but all letters presenent in used sequence
  if(!(is.null(additionalAA))){
    i = length(as.list(strsplit(additionalAA, '')[[1]]))
  }else{
    i = 0
  }
  N = 20 + i #length(namespace)
  ...
  # Information content
  attr(pfm_mat, 'bits') = computeBits(pfm_mat, N, nseqs, smallSampleCorr = smallSampleCorr)
...
bits_method <- function(seqs, decreasing, smallSampleCorr=TRUE, additionalAA=NULL, ...){
  # Make PFM
  pfm = makePFM(seqs, smallSampleCorr = smallSampleCorr, additionalAA = additionalAA)
