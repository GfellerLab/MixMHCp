...
list_fonts <- function(v=T){
  ...
  fonts = c('helvetica_phosphorylated') #,'helvetica_regular','helvetica_bold', 'helvetica_light', 
            #'roboto_medium','roboto_bold', 'roboto_regular', 'roboto_phosphorylated',
...
get_font <- function(font, libPath){
  ...
  if(is.null(font_obj)){
    ...
    font_path = file.path(paste0(libPath, 'ggseqlogoMOD'), font_filename)
...
logo_data <- function( seqs, method='bits', stack_width=0.95,
                       ...
                       smallSampleCorr=TRUE, additionalAA=NULL, libPath=NULL, ... ){
  ...
  if(method == 'bits'){
    hh = bits_method(seqs, decreasing = rev_stack_order, seq_type = seq_type, namespace = namespace, smallSampleCorr = smallSampleCorr, additionalAA = additionalAA)
...
geom_logo <- function(data = NULL, method='bits', seq_type='auto', namespace=NULL,
                      ...
                      plot=T,
                      smallSampleCorr=TRUE, additionalAA=NULL, legendText=FALSE, libPath=NULL, ...) {
  ...
  if(is.list(data)){
    ...
    data_sp = lapply(names(data), function(n){
      ...
      logo_data(seqs = curr_seqs, method = method, stack_width = stack_width,
                ...
                font = font, namespace = namespace, smallSampleCorr=smallSampleCorr, additionalAA=additionalAA, libPath=libPath)
    })
  ...
  list(logo_layer, #scale_x_continuous(breaks = breaks_fun, labels = identity, expand = c(0,0)),
